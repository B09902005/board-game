...
window.addEventListener("click", (event) => {
    ...
    if (event.target.id.includes("transfer_to") == true){
        if (event.target.id[11] == '0') transferto(playerData[who], '車站', 100);
        if (event.target.id[11] == '1') transferto(playerData[who], '廣場', 300);
        if (event.target.id[11] == '2') transferto(playerData[who], '終點', 500);
    }
